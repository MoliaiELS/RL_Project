...
def build_agent(args, obs_shape: tuple[int, int, int], n_actions: int):
    return TDLambdaCNNAgent(
        obs_shape=obs_shape,
        n_actions=n_actions,
        gamma=args.gamma,
        alpha=args.alpha,
...
def run_training(args):
    ...
    agent = build_agent(args, obs_shape, env.action_space.n)
    ...
    run_dir = os.path.join(args.save_dir, f"{timestamp}-tdlambda-cnn-{safe_env_id}")
    os.makedirs(run_dir, exist_ok=True)

    model_path = os.path.join(run_dir, f"tdlambda_cnn_{safe_env_id}.pt")
    ...
    plot_learning_curve(
        history,
        title=f"TD(λ) CNN on {args.env_id}",
        save_path=os.path.join(run_dir, f"tdlambda_cnn_learning_curve.png"),
    )
    if greedy_eval_history:
        ...
        plot_learning_curve(
            history,
            title=f"TD(λ) CNN Training and Greedy Eval on {args.env_id}",
            save_path=os.path.join(run_dir, f"tdlambda_cnn_combined_learning_curve.png"),
        ...
        plot_learning_curve(
            ...
            save_path=os.path.join(run_dir, f"tdlambda_cnn_greedy_eval_curve.png"),
        )
    ...
    print(f"Saved plot to {os.path.join(run_dir, f'tdlambda_cnn_learning_curve.png')}")
...
def parse_args():
    ...
    parser.add_argument("--no-manhattan-distance", action="store_false", dest="use_manhattan_distance", default=True,
