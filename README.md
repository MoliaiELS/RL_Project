...
**Usage:**
```bash
# Evaluate all models in a specific branch and save replays for GIF conversion
python eval/evaluate.py --model-path saved_models/action_features/ --eval-episodes 20 --save-replay
```

**Outputs per model folder:**
*   `eval_summary.json`: Contains Success Rate, Mean Reward, and Avg Steps.
*   `policy_heatmap.png`: A heatmap of $V(s) = \max_a Q(s,a)$ showing the agent's spatial awareness.
*   `/replay/episode_XX.npz`: Raw frames for video generation.

---

### 2. `visualize_model.py` (Real-time Demo & GIF)

**Usage:**
```bash
# Visualize a specific model and save as a high-res GIF
python eval/visualize_model.py --agent-type tdlambda --model-path path/to/model.npy --save-gif --gif-scale 40
```

---

### 3. `analyze_results.py` (Hyperparameter Analysis)
Parses the `search_summary.csv` generated by the grid search. It identifies the best performing parameters and generates:
*   **Lambda Sensitivity Plots**: Impact of $\lambda$ on reward.
*   **Alpha Stability Boxplots**: Robustness of the learning rate.
*   **LaTeX Tables**: Ready-to-use code for the project report.
...
### 4. `compare_methodology.py` (Methodology PK)
This script performs a direct comparison between the **Base Agent (Raw States)** and the **Enhanced Agent (Action Features)**. 

**Usage:**
```bash
python eval/compare_methodology.py
```
